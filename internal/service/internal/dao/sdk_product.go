// =================================================================================
// This is auto-generated by GoFrame CLI tool only once. Fill this file as you wish.
// =================================================================================

package dao

import (
	"github.com/gogf/gf-demo-user/v2/internal/service/internal/dao/internal"
)

// sdkProductDao is the data access object for table yjs_sdk_product.
// You can define custom methods on it to extend its functionality as you wish.
type sdkProductDao struct {
	*internal.SdkProductDao
}

var (
	// SdkProduct is globally public accessible object for table yjs_sdk_product operations.
	SdkProduct = sdkProductDao{
		internal.NewSdkProductDao(),
	}
)

// Fill with you ideas below.
